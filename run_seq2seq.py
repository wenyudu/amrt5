...
trainer = FlagAISpiderTrainer(
    ...
    gradient_accumulation_steps=10,  # 'Data Loader batch size'
    weight_decay=0.0,  # 'weight decay coefficient for L2 regularization'
    lr=1e-3,
    warm_up=0,
    epochs=500,  # 'Number of finetunning epochs. Zero results in evaluation only.'
    save_interval=900000000,  # 'number of epochs between saves')
    eval_interval=200, # do not do evaluate in trainer.train
